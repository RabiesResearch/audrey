...
import { getAllRegionsAndDistricts, getPatientAndStockNumbers } from "$lib/data/api";
...
export const POST: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    const { selectedRegions, selectedDistricts, format = "csv" } = await request.json();
    ...
    if (allowedRegionNames.length > 0) {
      ...
      if (unauthorizedRegions.length > 0) {
        return json({ error: "Access denied for some selected regions" }, { status: 403 });
      }
    }
